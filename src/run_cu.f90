...
!     READ(1,fmt='(*,*,A)',ERR=119) N, CROP_TYPE(K), CPNAME(K)
  ...
  DO K=1,N_CROPS
	...
	CALL MYEXIT(145)
     ENDIF
     IF( MAD(N) .GT. 100 ) THEN
        PRINT *, "Illegal MAD value for crop ", CNAME(N)
	CALL MYEXIT(-1)
     ENDIF
	...
	DO J = 1,NYRS
	   ...
	   if (SUB_CROP_TYPE(CROP_KEY(I,K)) .EQ. 0) THEN
	      ! Assign area to synthetic spring period winter wheat parcel.
              !   use the previous year's acreage
              if (J+NYR1-GNYR1-1 .gt. 0) then
                 AREA(I,NPARCE(I)+n_wheat_parcel,J) = GAREA(J+NYR1-GNYR1-1)
              endif
	   endif
	END DO
     END DO
