SUBROUTINE DAYHRS(LT,DHRS)
  ...
  !   Author          : HB Manguerra
  !   Date            : December 1994
  !   Purpose         : This calculates the monthly percent daylight hours
  !                     associated with a given latitude.  A table of monthly
  !                     values pf percent daylight hours with respect to
  !                     selected latitudes (0-64 degree North) is used.
  !   Calling program : mainxc.f, summary.f, other.f 
  !   Called programs : none 
  !   Input arguments : lt   = latitude
  !   Output arguments: dhrs() = monthly daylight hours
  !   Assumptions     : It takes the latitude assigned to the cropped area.
  !                     An alternative is to use the latitude of the weather
  !                     station (or weighted average of the latitudes of
  !                     several weather stations.
  !   Limitations     : The table of values are only for latitudes 0-64 degree
  !                     North. 
  !   Notes           : The table of monthly daylight hours is taken from
  !                     the ASCE Manuals and Reports No. 70 on Evapotranspi-
  !                     ration and Irrigation Water Requirements by Jensen et
  !                     al., 1990.
  !
  !***************************************************************************
  ...
  INTEGER DOY, M, D
  REAL LT, DHRS(12)
  REAL LAT_rad, Ws_ang, SD, Hi, Hj, SUM_Hi
  ...
  IF ( (LT .GT. 65.0) .OR. (LT .LE. 0.0)) CALL MYEXIT(59)
  ...
  DO M = 1,12
	...
	LAT_rad = (PI/180.0) * LT     
	Ws_ang = ACOS(-TAN(LAT_rad)*TAN(SD))
	Hi = (24.0/PI) * Ws_ang
	SUM_Hi = SUM_Hi + Hi 
     END DO
  END DO
  ...
  DO M = 1,12
	...
	Ws_ang = ACOS(-TAN(LAT_rad)*TAN(SD))
	Hj = (24.0/PI) * Ws_ang
	DHRS(M) = DHRS(M) + ((Hj/SUM_Hi)*100.0) 
     END DO
  END DO

  RETURN
END SUBROUTINE DAYHRS
