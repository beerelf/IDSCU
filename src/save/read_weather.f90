SUBROUTINE READ_WEATHER(et_method)
  ...
  INTERFACE
     ...
     SUBROUTINE ETREF(is,ndyr,minT,maxT,dpt,wd,rs,rh_mean,s_ZM,s_ZH, &
	  ...
	  etr_y,eto_y)
       ...
       REAL s_ZM, s_ZH, RS(366), RH_MEAN(366)
  ...
  INTEGER IERR, IERR1
  REAL X1(366),X2(366),X3(366),X4(366),X5(366),X6(366), X7(366)
  REAL dummy, SWWS
  CHARACTER*80 REMARK
  CHARACTER*320 LINE1
  ...
  !Local Penmans
  REAL maxT(366), minT(366), DPT(366), RS(366), WD(366), RH_MEAN(366)
 ...
 !--------------Read the temperature and rainfall data
    ...
    !-------------------------------------------------------------
	   ...
	   ELSE
	      READ(18,*,ERR=114) DAY, X1(iper),X2(iper), &
		   X3(iper),X4(iper),X5(iper),X6(iper), X7(iper)
    ...
    !----------------------------
	   IF (et_method.EQ.2 .or. et_method.eq.4 .or.  &
		et_method .EQ. 6 .or. et_method.eq.10) THEN
...
9000	      FORMAT("Computing PM ET reference for ",A10," year ",I4)
	      CALL ETREF(i,nperiods, &
		   minT,maxT,dpt,wd,rs,rh_mean,ZM(i),ZH(i),slat(i),elev(i), &
 ...
 ! Convert illegal values to zero.
  ...
  if (et_method .EQ. 9) then
	...
	DO iy = 1,nyrs
	      ...
	      nperiods = 12
           ELSE
	      nperiods = DaysInYear(IY + NYR1 -1)
           ENDIF

	   DO IB = 1, NBASIN
              DO iper=1,nperiods
		 call ReadCU(ib, iy, etr(ib, :, :))
                 call ReadCU(ib, iy, eto(ib, :, :))
	      END DO
           END DO
	END DO
     END IF
