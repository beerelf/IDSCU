SUBROUTINE DISTR(IB,IY,L,nperiods,exces_er,cuirr,Qs,Qr,pc_rz,nendmo, &
 ...
 !water storage has (below zero).
        ...
        IF (USE_WATER_STRESS) THEN
	       ...
	       Qreq = (storag(IP)/12)*area(IB,IP,IY)
	    END IF
	ELSE
            IF (storag(IP) .LT. 0.0) THEN
 ...
 !with soil moisture storage still < Scap
  ...
  ELSEIF( Qs.GE. 0.0 .AND. SMBSeason .EQ. 0) THEN            ! IF Qs < Qr
	   ...
	   IF ((storag(IP)+Qs_UD_p(IP)-cuirr_sh).LT.Scap(IP)) THEN
		 ...
		 deficit(IP) = 0.0
	      ENDIF
              ...
              IF (USE_WATER_STRESS) THEN
		 ! Include deficit in water storage so that it will go negative
		 storag(IP)=storag(IP)+Qs_UD_p(IP)-cuirr_sh + deficit(IP)
              ELSE
	...
	IF (cuirr_sh .GT. 0.0001 ) THEN
	   IF (storag(IP).LT.0.0) THEN
              IF (USE_WATER_STRESS) THEN
		 sh = (-1.0*storag(IP))*AREA(IB,IP,IY)
	      ELSE
