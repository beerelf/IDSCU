SUBROUTINE READ_CROP_BC()
  ...
  !***************************************************************************
  !
  !   Function        : read_crop_bc.f
  !   Author          : LA Garcia & Dave Patterson
  !   Date            : dec 2002
  !   Purpose         : This reads the Blaney-Criddle crop coefficient file
  ...
  DO i=1,NC 
     READ(4,*,ERR=101) ID, crptyp

     !-------------------------------------------------------------
     !     Read the Calibrated Blaney-Criddle Monthly Coefficients
     !-------------------------------------------------------------
     READ(4,*) (CBC(ID,J),J=1,12)

     IF (crptyp.eq.0 .OR. crptyp.eq.1 .OR. crptyp.eq.3) THEN  ! alfalfa or perennial or phreatophyte.
	DO j=1,PKCLEN 
