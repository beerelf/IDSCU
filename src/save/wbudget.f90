SUBROUTINE WBUDGET(et_method)
  ...
  REAL totwsup, totrain, effrain, erstor, iwr, csoilm 
  ...
  REAL sdepsw, sadddpro, stotdpro, scuground, serstor, sdprorain
	   ...
	   DO I = 1, nperiods(IY)
		 ...
		 label = amn(I)
	      ...
	      READ(12,*) rsup,conloss,fssup,swacu,swdpro,fwelsup, &
		   ...
		   sprink_l, short_reduc, cae, short_stress
...
902 FORMAT(    1x,"Month    River      Conveyance    Farm Surface Surf&
       ...
       &l DP &    CU Met by    CU Met by    Rootzone    Max Storage   Sprinkler Loss    NWR Short    Crop Stress"/ &
...
903 FORMAT(1x,A1,A6,A1,1x,2(F12.3,1x),2x,25(F12.3,1x))
904 FORMAT(A1,'Total',A1,2(F12.3,1x),2x,25(F12.3,1x))
...
913 FORMAT(1x,A1,A6,A1,1x,2(F12.3,1x),2x,9(F12.3,1x), &
	 2(A1,1x,A10,1x,A1),F12.3,1x,A1,1x,A10,1x,A1,6(F12.2,1x), &
	 2(A1,1x,A10,1x,A1),4(F12.2,1x))
914 FORMAT(A1,'Total',A1,2(F12.3,1x),2x,9(F12.3,1x), &
	 2(A1,1x,A10,1x,A1),F12.3,1x,A1,1x,A10,1x,A1,6(F12.2,1x), &
	 2(A1,1x,A10,1x,A1), 4(F12.2,1x))
