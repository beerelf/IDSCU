...
!***************************************************************************
     ...
     REAL awcmad, totarea!, excess_er_acft
...
198     END DO
           ...
           DO IP=1,NPARCE(IB)
              DO iper = 1,NDYR
                 ...
                 CALL FOUTPUT(IP, nyr, IB, iper, NYR1 + nyr -1, &
                      ...
                      CU(IP, iper), ER(IP, iper), exces_er(IP,MM), 0., NDYR)
              END DO
           END DO
        end if
        ...
        IF (TYPOUT(IB).EQ.3) THEN
           DO IP = 1, nparce(IB)
                 ...
                 IF (MM.EQ.nbegmo(IP)) THEN
                    IF(SPFLAG .EQ. 1 .AND. et_method.EQ.7) THEN
                       WRITE(3,911) QUOTE,AMN(MM),nbegda(IP),QUOTE, &
                            ...
                            Ret(IP,MM), &
                            xxkc(IP,MM),cu(IP,MM),er(IP,MM)+exces_er(IP,MM),cuirr(IP,MM)
                    ELSEIF(et_method.EQ.1 .OR. et_method.EQ.8) THEN
                       WRITE(3,901) QUOTE,AMN(MM),nbegda(IP),QUOTE, &
                            100.*npart(IP)/days,ttemps(IP),ddays(IP), &
                            xxf(IP,MM),xxkt(IP,MM),xxkc(IP,MM), &
                            xxkt(IP,MM)*xxkc(IP,MM),cu(IP,MM),er(IP,MM)+exces_er(IP,MM), &
                            cuirr(IP,MM)
                    ELSEIF(et_method.EQ.5)THEN
                       WRITE(3,921) QUOTE,AMN(MM),nbegda(IP),QUOTE, &
                            ...
                            er(IP,MM)+exces_er(IP,MM),cuirr(IP,MM)
                    ELSEIF(et_method.EQ.9) THEN
                       ! Add user-supplied ET
                    ENDIF
                 ...
                 ELSE
                    IF(SPFLAG.EQ.1 .AND. et_method.EQ.7) THEN
                       WRITE(3,912)QUOTE,AMN(MM),QUOTE, &
                            ...
                            xxkc(IP,MM),cu(IP,MM),er(IP,MM)+exces_er(IP,MM), &
                            cuirr(IP,MM)
                    ELSEIF(et_method.EQ.1 .OR. et_method.EQ.8)THEN
                    ...
                    ELSEIF(et_method.EQ.5)THEN
                       WRITE(3,922)QUOTE,AMN(MM),QUOTE, &
                            ...
                            cu(IP,MM),er(IP,MM)+exces_er(IP,MM), cuirr(IP,MM)
                    ELSEIF(et_method.EQ.9) THEN
                       ! Add user-supplied ET
                    ENDIF
                 ENDIF
              ...
              IF(SPFLAG.EQ.1 .AND. et_method.EQ.7) THEN
                 WRITE(3,916) SLINE
                 WRITE(3,914) (QUOTE, IDUM=1,16),cu(IP,13),er(IP,13)+exces_er(IP,13), cuirr(IP,13)
              ELSEIF(et_method.EQ.1 .OR. et_method.EQ.8) THEN
                 WRITE(3,906) SLINE
                 WRITE(3,904) (QUOTE, IDUM=1,16),cu(IP,13),er(IP,13)+exces_er(IP,13), cuirr(IP,13)
              ELSEIF(et_method.EQ.5) THEN
                 WRITE(3,906) SLINE
                 WRITE(3,924) (QUOTE, IDUM=1,16),cu(IP,13),er(IP,13)+exces_er(IP,13), cuirr(IP,13)
              ELSEIF(et_method.EQ.9) THEN
                 ! Add user-supplied ET
              ENDIF
              WRITE(3,*)
